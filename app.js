...
function RobotSchoolDashboard() {
    ...
    // --- States (Error Fix: isCampusMenuOpenを定義) ---
    ...
    const generateAllCampusesData = (targetCampuses = [], enrollList = [], statusList = [], transferList = [], reportList = [], trialAppsList = [], targetYear) => {
            ...
            dataMap[campusId] = MONTHS_LIST.map((month, mIdx) => {
                ...
                const daily = Array.from({ length: daysInMonth }, (_, dIdx) => {
                    ...
                    trialAppsList.forEach(app => {
                        ...
                        const aD = parseDate(app.date); 
                        if (aD && formatDateStr(aD) === dateStr) { if(isEvent) eApp++; else tApp++; }
                        const eD = parseDate(app.trialDate); 
                        if (eD && formatDateStr(eD) === dateStr) { if(isEvent) eExec++; else tExec++; }
                    });
                ...
                const monthObj = { name: month, totalStudents: currentStudents, daily, weekly };
                Object.keys(daily[0]).forEach(k => { if(k !== 'name') monthObj[k] = mSum(k); });
                return monthObj;
            });
        });
        ...
        dataMap['All'] = MONTHS_LIST.map((m, idx) => {
            ...
            baseKeys.forEach(k => {
                comb[k] = 0;
                comb.daily.forEach(d => d[k] = 0);
                comb.weekly.forEach(w => w[k] = 0);
            });
    ...
    useEffect(() => {
        if (campusList.length) {
            setRawDataMap(generateAllCampusesData(campusList, realEnrollments, realStatusChanges, realTransfers, realDailyReports, realTrialApps, selectedYear));
        }
        ...
        <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900">
            ...
            <main className="flex-1 flex flex-col overflow-hidden relative">
                <header className="bg-white border-b h-16 flex items-center justify-between px-6 shrink-0">
                    <div>
                        <h1 className="text-xl font-bold text-slate-800">{{summary:'経営サマリー', students:'生徒数管理', marketing:'集客・販促', campus_daily:'日報入力', campus_yearly:'計画入力', settings:'校舎設定'}[activeTab]}</h1>
                        ...
                        <div className="bg-white rounded-xl shadow-2xl max-w-md w-full animate-in zoom-in-95">
                            ...
                            <div className="p-6 space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="text-xs font-bold block mb-1">門配 (枚)</label><input type="number" disabled={dailyReportInput.weather==='closed'} className="border rounded w-full p-2 text-right" value={dailyReportInput.flyers} onChange={e=>setDailyReportInput({...dailyReportInput,flyers:Number(e.target.value)})}/></div>
                                    <div><label className="text-xs font-bold block mb-1">T&T (件)</label><input type="number" disabled={dailyReportInput.weather==='closed'} className="border rounded w-full p-2 text-right" value={dailyReportInput.touchTry} onChange={e=>setDailyReportInput({...dailyReportInput,touchTry:Number(e.target.value)})}/></div>
                                </div>
