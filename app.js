...
function RobotSchoolDashboard() {
    ...
    const calculateData = () => {
        ...
        campusList.forEach(campusObj => {
            ...
            dataMap[campusId] = MONTHS_LIST.map((month, mIdx) => {
                ...
                const daily = Array.from({ length: daysInMonth }, (_, dIdx) => {
                    ...
                    const dEnr = countOnDay(realEnrollments); const dTrIn = countOnDay(realTransfers);
                    const dWith = countOnDay(realStatusChanges, "退会"); const dTrOut = countOnDay(realStatusChanges, "転校");
                    const dGrad = countOnDay(realStatusChanges, "卒業"); const dRec = countOnDay(realStatusChanges, "休会");
        ...
        dataMap['All'] = MONTHS_LIST.map((m, idx) => {
            ...
            const base = { name: m, newEnrollments:0, transferIns:0, withdrawals:0, recesses:0, returns:0, transfers:0, graduates:0, totalStudents:0, flyers:0, touchAndTry:0, trialApp:0, eventApp:0, trialExec:0, eventExec:0, withdrawals_neg:0, recesses_neg:0, transfers_neg:0, graduates_neg:0 };
            const comb = { ...base, daily: Array.from({length:daysInMonth}, (_,i)=>({name:`${i+1}日`, ...base})), weekly: weeks.map(w=>({name:w.name, ...base})) };
            campusList.forEach(c => {
                const d = dataMap[c.id]?.[idx];
                if (d) {
                    Object.keys(base).forEach(k => { if(typeof comb[k]==='number') comb[k] += d[k]; });
                    d.daily.forEach((day, i) => { if(comb.daily[i]) Object.keys(base).forEach(k => { if(typeof day[k]==='number' && k!=='name') comb.daily[i][k] += day[k]; })});
                    d.weekly.forEach((wk, i) => { if(comb.weekly[i]) Object.keys(base).forEach(k => { if(typeof wk[k]==='number' && k!=='name') comb.weekly[i][k] += wk[k]; })});
                }
            });
            return comb;
        });
    ...
    useEffect(() => {
        if (campusList.length) setRawDataMap(calculateData());
        ...
        <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900">
                                ...
                                <div key={c.id} className="ml-2">
                                    ...
                                    {expandedCampusId === c.id && (
                                        <div className="ml-4 pl-2 border-l border-slate-700 mt-1 space-y-1">
                                            ...
                                            <button onClick={() => handleMenuClick('campus_yearly', c.id)} className={`w-full text-left px-3 py-1.5 text-xs rounded ${activeTab==='campus_yearly'&&selectedCampusId===c.id?'text-blue-400 font-bold':'text-slate-500'}`}>└ 年間計画入力</button>
                                        </div>
                                    )}
                                </div>
                            ))}
            ...
            <main className="flex-1 flex flex-col overflow-hidden relative">
                <header className="bg-white border-b h-16 flex items-center justify-between px-6 shrink-0">
                    <div>
                        <h1 className="text-xl font-bold text-slate-800">{{summary:'経営サマリー', students:'生徒数・入退会管理', marketing:'集客・販促', campus_daily:'日報入力', campus_yearly:'年間計画入力', settings:'校舎設定'}[activeTab]}</h1>
                        <p className="text-[10px] text-slate-500">{selectedCampusName}</p>
                    </div>
                    <div className="flex items-center space-x-3">
                        ...
                        <div className="flex items-center bg-slate-100 rounded-lg px-3 py-1 border border-slate-200">
                            ...
                            <select value={selectedCampusId} onChange={(e) => setSelectedCampusId(e.target.value)} className="bg-transparent border-none text-sm font-medium text-slate-700 focus:ring-0 cursor-pointer py-1"><option value="All">全校舎合計</option>{campusList.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
                        </div>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    {activeTab === 'summary' && (
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 animate-in fade-in">
                            <StatCard title="在籍生徒数" value={`${currentTotalStudents}名`} subValue="当期末時点" icon={Users} color="bg-indigo-500" />
                            <StatCard title="門配累計" value={`${totals.flyers}枚`} subValue="選択期間" icon={Megaphone} color="bg-orange-500" />
                            <StatCard title="体験申込数" value={`${totals.trialApp + totals.eventApp}名`} subValue="累計" icon={Calendar} color="bg-blue-500" />
                            <StatCard title="新規入会数" value={`${totals.newEnrollments}名`} subValue="新規" icon={Activity} color="bg-emerald-500" />
                        </div>
                    )}
                    ...
                    {activeTab === 'settings' && (
                        <div className="bg-white p-8 rounded-xl border max-w-2xl mx-auto animate-in fade-in">
                            <h2 className="text-lg font-bold mb-6 flex items-center"><School className="mr-2"/>校舎追加</h2>
                            <div className="flex gap-4 items-end mb-8">
                                <div className="flex-1"><label className="text-xs block mb-1">ID (英数)</label><input className="border w-full p-2 rounded-lg" value={newCampusId} onChange={e=>setNewCampusId(e.target.value)} placeholder="shibuya" /></div>
                                <div className="flex-1"><label className="text-xs block mb-1">校舎名</label><input className="border w-full p-2 rounded-lg" value={newCampusName} onChange={e=>setNewCampusName(e.target.value)} placeholder="渋谷本店" /></div>
                                <button onClick={handleAddCampus} className="bg-blue-600 text-white px-6 py-2 rounded-lg h-10 hover:bg-blue-700">登録</button>
                            </div>
                            <div className="space-y-2">
                                {campusList.map(c => (<div key={c.id} className="p-3 bg-slate-50 rounded-lg flex justify-between items-center border"><span><b>{c.name}</b> (ID:{c.id})</span><button onClick={()=>handleDeleteCampus(c.id, c.name)} className="text-red-500 p-2 hover:bg-red-50 rounded-full"><Trash2 size={16}/></button></div>))}
                            </div>
                        </div>
                    )}

                    {activeTab === 'campus_yearly' && (
                        <div className="bg-white p-6 rounded-xl border animate-in fade-in">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="font-bold flex items-center"><FileText className="mr-2"/>年間計画入力 ({selectedCampusName})</h2>
                                <button onClick={savePlanData} disabled={isSavingPlan} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center">{isSavingPlan ? <Loader2 className="animate-spin mr-1"/> : <Save className="mr-1"/>} 保存</button>
                            </div>
                            <table className="w-full text-sm">
                                <thead className="bg-slate-50"><tr><th className="p-2 text-left">月</th><th>入会目標</th><th>体験目標</th><th>門配目標</th></tr></thead>
                                <tbody>
                                    {MONTHS_LIST.map(m => (
                                        <tr key={m} className="border-b">
                                            <td className="p-2 font-bold">{m}</td>
                                            {['enrollments','trials','flyers'].map(f => (
                                                <td key={f} className="p-1"><input type="number" className="w-full border rounded p-1 text-right" value={planData[m]?.[f] || 0} onChange={e => handlePlanChange(m, f, e.target.value)} /></td>
                                            ))}
                                        </tr>
                                    ))}
                                </tbody>
                        ...
                        <div className="bg-white rounded-xl shadow-2xl max-w-md w-full animate-in zoom-in-95">
                            ...
                            <div className="p-6 space-y-6">
                                <div><label className="text-sm font-bold mb-2 block">状態/天気</label>
                                    <div className="grid grid-cols-5 gap-2">
                                        {[{id:'sunny',l:'晴',i:Sun,c:'text-orange-500'},{id:'cloudy',l:'曇',i:Cloud,c:'text-gray-500'},{id:'rainy',l:'雨',i:CloudRain,c:'text-blue-500'},{id:'snowy',l:'雪',i:Snowflake,c:'text-cyan-500'},{id:'closed',l:'休校',i:Ban,c:'text-red-500'}].map(w=>(
                                            <button key={w.id} onClick={()=>handleWeatherSelect(w.id)} className={`flex flex-col items-center p-2 border rounded-lg transition-all ${dailyReportInput.weather===w.id?'bg-blue-50 border-blue-500 ring-2 ring-blue-500':'hover:bg-slate-50'}`}><w.i className={`w-5 h-5 ${w.c} mb-1`} /><span className="text-[10px] font-bold">{w.l}</span></button>
                                        ))}
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="text-xs font-bold text-slate-500 mb-1 block">門配 (枚)</label><input type="number" disabled={dailyReportInput.weather==='closed'} className="border rounded w-full p-2 text-right font-mono text-lg" value={dailyReportInput.flyers} onChange={e=>setDailyReportInput({...dailyReportInput,flyers:Number(e.target.value)})}/></div>
                                    <div><label className="text-xs font-bold text-slate-500 mb-1 block">T&T (件)</label><input type="number" disabled={dailyReportInput.weather==='closed'} className="border rounded w-full p-2 text-right font-mono text-lg" value={dailyReportInput.touchTry} onChange={e=>setDailyReportInput({...dailyReportInput,touchTry:Number(e.target.value)})}/></div>
                               </div>
                                <button onClick={handleSaveDailyReport} disabled={isSavingReport} className="w-full bg-blue-600 text-white py-3 rounded-lg font-bold shadow hover:bg-blue-700 transition-all flex items-center justify-center disabled:opacity-50">
                                    {isSavingReport ? <Loader2 className="animate-spin mr-2"/> : <Save className="mr-2"/>} 保存
                                </button>
                            </div>
                        </div>
                    </div>
