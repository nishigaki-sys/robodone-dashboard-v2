...
function RobotSchoolDashboard() {
    ...
    const loadFromCache = () => {
        try {
            ...
            if (cachedCampuses && cachedEnroll && cachedStatus && cachedTransfers) {
                ...
                if (cachedTrialApps) setRealTrialApps(JSON.parse(cachedTrialApps)); 
    ...
    const generateAllCampusesData = (targetCampuses, realEnrollmentList, realStatusList, realTransferList, dailyReportsList, trialAppsList, targetYear) => {
        ...
        const getResolvedCampusId = (rawName) => {
            ...
            const match = targetCampuses.find(c => {
                ...
                // 登録名が入力データに含まれているか、あるいは入力データが登録名に含まれているかを確認
                return normInput.includes(normCName) || normInput.includes(normCSheet) || 
                       normCName.includes(normInput) || normCSheet.includes(normInput);
            });
        ...
        trialAppsList.forEach(app => {
            const campusId = getResolvedCampusId(app.campus);
            if (!campusId) return;
        ...
        const countTotalBefore = (list, year, typeFilter = null) => {
            ...
            list.forEach(item => {
                ...
                const campusId = getResolvedCampusId(item.campus);
                if (!campusId) return;
        ...
        targetCampuses.forEach(campusObj => {
            ...
            dataMap[campusId] = MONTHS_LIST.map((month, mIdx) => {
                ...
                const weekly = weeks.map(week => {
                    ...
                    return {
                        name: week.name, budgetRevenue: 0, actualRevenue: 0,
                        newEnrollments: wVal.enroll, transferIns: wVal.transferIn, withdrawals: wVal.withdraw, recesses: wVal.recess, returns: wVal.return, transfers: wVal.transfer, graduates: wVal.graduate,
                        flyers: wVal.flyers, touchAndTry: wVal.touch, trialApp: wVal.trialApp, eventApp: wVal.eventApp, trialExec: wVal.trialExec, eventExec: wVal.eventExec,
                        totalStudents: currentStudents, withdrawals_neg: -wVal.withdraw, recesses_neg: -wVal.recess, transfers_neg: -wVal.transfer, graduates_neg: -wVal.graduate
                    };
                });
                ...
                daily.forEach(d => { mFlyers += d.flyers; mTouch += d.touchAndTry; mTrialApp += d.trialApp; mEventApp += d.eventApp; mTrialExec += d.trialExec; mEventExec += d.eventExec; });

                return {
                    name: month, budgetRevenue: 0, actualRevenue: 0,
                    newEnrollments: val.enroll, transferIns: val.transferIn, withdrawals: val.withdraw, recesses: val.recess, returns: val.return, transfers: val.transfer, graduates: val.graduate,
                    totalStudents: currentStudents, flyers: mFlyers, touchAndTry: mTouch, trialApp: mTrialApp, eventApp: mEventApp, trialExec: mTrialExec, eventExec: mEventExec,
                    withdrawals_neg: -val.withdraw, recesses_neg: -val.recess, transfers_neg: -val.transfer, graduates_neg: -val.graduate, daily, weekly
                };
            });
        });
        ...
        dataMap['All'] = MONTHS_LIST.map((month, idx) => {
            ...
            const combined = {
                name: month, budgetRevenue: 0, actualRevenue: 0, newEnrollments: 0, transferIns: 0, withdrawals: 0, recesses: 0, returns: 0, transfers: 0, graduates: 0, totalStudents: 0, flyers: 0, touchAndTry: 0, trialApp: 0, eventApp: 0, trialExec: 0, eventExec: 0, withdrawals_neg: 0, recesses_neg: 0, transfers_neg: 0, graduates_neg: 0,
                daily: Array.from({ length: daysInMonth }, (_, i) => ({ name: `${i+1}日`, newEnrollments:0, transferIns:0, returns:0, withdrawals:0, recesses:0, transfers:0, graduates:0, flyers: 0, touchAndTry: 0, trialApp: 0, eventApp: 0, trialExec: 0, eventExec: 0, withdrawals_neg: 0, recesses_neg: 0, transfers_neg: 0, graduates_neg: 0 })),
                weekly: weeks.map(w => ({ name: w.name, newEnrollments:0, transferIns:0, returns:0, withdrawals:0, recesses:0, transfers:0, graduates:0, flyers: 0, touchAndTry: 0, trialApp: 0, eventApp: 0, trialExec: 0, eventExec: 0, withdrawals_neg: 0, recesses_neg: 0, transfers_neg: 0, graduates_neg: 0 }))
            };
            targetCampuses.forEach(campusObj => {
                const d = dataMap[campusObj.id]?.[idx];
                if (d) {
                    Object.keys(combined).forEach(k => { if (typeof combined[k] === 'number') combined[k] += d[k]; });
                    d.daily.forEach((day, i) => { if (combined.daily[i]) { Object.keys(day).forEach(k => { if(typeof day[k] === 'number' && k!=='name') combined.daily[i][k] = (combined.daily[i][k]||0) + day[k]; }); } });
                    d.weekly.forEach((wk, i) => { if (combined.weekly[i]) { Object.keys(wk).forEach(k => { if(typeof wk[k] === 'number' && k!=='name') combined.weekly[i][k] = (combined.weekly[i][k]||0) + wk[k]; }); } });
                }
            });
            return combined;
        });
    ...
    const totals = useMemo(() => {
        ...
        return displayData.reduce((acc, curr) => ({
            ...
            trialLessons: acc.trialLessons + (curr.trialLessons || 0), // ★追加
    ...
    const handleDateClick = (day) => {
        ...
        if (existingReport) {
            ...
            setDailyReportInput({ weather: existingReport.weather || 'sunny', touchTry: isClosed ? 0 : (existingReport.touchTry || 0), flyers: isClosed ? 0 : (existingReport.flyers || 0), trialLessons: isClosed ? 0 : (existingReport.trialLessons || 0) });
        } else { setDailyReportInput({ weather: 'sunny', touchTry: 0, flyers: 0, trialLessons: 0 }); }
    ...
    const handleMenuClick = (tab, campusId = 'All') => { setActiveTab(tab); setSelectedCampusId(campusId); };
    const toggleCampusMenu = (campusId) => { setExpandedCampusId(expandedCampusId === campusId ? null : campusId); };
    const handleWeatherSelect = (weatherId) => {
        if (weatherId === 'closed') { setDailyReportInput({ ...dailyReportInput, weather: weatherId, flyers: 0, touchTry: 0, trialLessons: 0 }); } 
        else { setDailyReportInput({ ...dailyReportInput, weather: weatherId }); }
    };
    ...
    const renderCalendar = () => {
        ...
        const days = Array.from({ length: daysInMonth }, (_, i) => {
            ...
            return (
                <div key={day} onClick={() => handleDateClick(day)} className={`h-24 border border-slate-200 p-1.5 cursor-pointer hover:bg-blue-50 transition-colors relative flex flex-col ${isToday ? 'bg-blue-50/50' : isClosed ? 'bg-slate-100' : 'bg-white'}`}>
                    <div className="flex justify-between items-start mb-1"><span className={`text-sm font-bold ${isToday ? 'text-blue-600' : 'text-slate-700'}`}>{day}</span>{WeatherIcon && <WeatherIcon className={`w-4 h-4 ${WeatherInfo.c}`} />}</div>
                    ...
                    {report ? (
                        <div className="flex-1 flex flex-col justify-end gap-0.5 overflow-hidden">
                            {isClosed ? (<div className="flex-1 flex items-center justify-center text-xs font-bold text-slate-400">休校</div>) : (
                    ...
                    ) : (<div className="flex-1 flex items-center justify-center text-slate-300"><Plus className="w-4 h-4 opacity-0 group-hover:opacity-100" /></div>)}
                </div>
            );
        });
        return [...blanks, ...days];
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900">
            <aside className="w-64 bg-slate-900 text-white flex flex-col shrink-0 overflow-y-auto">
                <div className="p-6 border-b border-slate-800"><div className="flex items-center space-x-2"><div className="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center"><TrendingUp className="w-5 h-5 text-white" /></div><span className="text-lg font-bold tracking-tight">RobotSchool<span className="text-blue-400">Dash</span></span></div></div>
                <nav className="flex-1 py-4 px-3 space-y-1">
                    <button onClick={() => handleMenuClick('summary')} className={`w-full flex items-center space-x-3 px-3 py-3 rounded-lg transition-colors ${activeTab === 'summary' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}><LayoutDashboard className="w-5 h-5" /><span className="font-medium">経営サマリー</span></button>
                    <button onClick={() => handleMenuClick('students')} className={`w-full flex items-center space-x-3 px-3 py-3 rounded-lg transition-colors ${activeTab === 'students' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}><Users className="w-5 h-5" /><span className="font-medium">生徒管理</span></button>
                    <button onClick={() => handleMenuClick('marketing')} className={`w-full flex items-center space-x-3 px-3 py-3 rounded-lg transition-colors ${activeTab === 'marketing' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}><Megaphone className="w-5 h-5" /><span className="font-medium">集客・販促</span></button>
                    <div className="pt-4 pb-2 px-3 text-xs font-semibold text-slate-500 uppercase tracking-wider flex justify-between cursor-pointer hover:text-slate-300" onClick={() => setIsCampusMenuOpen(!isCampusMenuOpen)}><span>校舎管理</span><ChevronDown className={`w-4 h-4 transition-transform ${isCampusMenuOpen ? 'rotate-180' : ''}`} /></div>
                    {isCampusMenuOpen && (<div className="space-y-1">
                            <button onClick={() => handleMenuClick('settings')} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors ${activeTab === 'settings' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}><Plus className="w-4 h-4" /><span className="text-sm">校舎追加</span></button>
                            {campusList.map(campus => (<div key={campus.id} className="ml-2">
                                    <button onClick={() => toggleCampusMenu(campus.id)} className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-slate-400 hover:bg-slate-800 ${expandedCampusId === campus.id ? 'text-white bg-slate-800' : ''}`}><div className="flex items-center"><Building className="w-4 h-4 mr-2" /><span className="text-sm">{campus.name}</span></div><ChevronRight className={`w-3 h-3 transition-transform ${expandedCampusId === campus.id ? 'rotate-90' : ''}`} /></button>
                                    {expandedCampusId === campus.id && (<div className="ml-4 pl-2 border-l border-slate-700 mt-1 space-y-1">
                                            <button onClick={() => handleMenuClick('campus_daily', campus.id)} className={`w-full text-left px-3 py-1.5 text-xs rounded transition-colors ${activeTab === 'campus_daily' && selectedCampusId === campus.id ? 'text-blue-400 font-bold' : 'text-slate-500 hover:text-slate-300'}`}>└ 日報入力</button>
                                            <button onClick={() => handleMenuClick('campus_weekly', campus.id)} className={`w-full text-left px-3 py-1.5 text-xs rounded transition-colors ${activeTab === 'campus_weekly' && selectedCampusId === campus.id ? 'text-blue-400 font-bold' : 'text-slate-500 hover:text-slate-300'}`}>└ 月度計画入力</button>
                                            <button onClick={() => handleMenuClick('campus_yearly', campus.id)} className={`w-full text-left px-3 py-1.5 text-xs rounded transition-colors ${activeTab === 'campus_yearly' && selectedCampusId === campus.id ? 'text-blue-400 font-bold' : 'text-slate-500 hover:text-slate-300'}`}>└ 年間計画入力</button>
                                        </div>)}
                                </div>))}
                        </div>)}
                </nav>
                <div className="p-4 border-t border-slate-800 text-xs text-slate-400 space-y-2 mt-auto"><div className="flex items-center justify-between"><div className="flex items-center"><Database className={`w-3 h-3 mr-1 ${isFirebaseInitialized ? 'text-emerald-400' : 'text-slate-500'}`} />{isFirebaseInitialized ? (isUsingCache ? 'Local Cache' : 'Firebase') : 'Local Mode'}</div></div>{lastUpdated && <div className="text-slate-500">更新: {lastUpdated.toLocaleTimeString()}</div>}</div>
            </aside>
            <main className="flex-1 flex flex-col overflow-hidden h-screen relative">
                ...
                <header className="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-6 sticky top-0 z-10 shrink-0">
                    <div><h1 className="text-xl font-bold text-slate-800">{{summary:'経営サマリー', students:'生徒数・入退会管理', marketing:'集客活動・販促管理', campus_daily:'日報入力 (カレンダー)', campus_weekly:'月度計画入力 (週次)', campus_yearly:'年間計画入力 (月度)', settings:'校舎設定・追加'}[activeTab]}</h1><p className="text-xs text-slate-500 mt-0.5 flex items-center">{selectedCampusId !== 'All' ? <><Building className="w-3 h-3 mr-1"/> {selectedCampusName}</> : '全校舎合計'}</p></div>
                    <div className="flex items-center space-x-3">{activeTab !== 'settings' && activeTab !== 'campus_weekly' && (<><div className="flex items-center bg-slate-100 rounded-lg px-3 py-1 border border-slate-200"><span className="text-xs text-slate-500 mr-2 font-bold">年度</span><select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} className="bg-transparent border-none text-sm font-medium text-slate-700 focus:ring-0 cursor-pointer py-1">{YEARS_LIST.map(y => <option key={y} value={y}>{y}年度</option>)}</select></div>
                                {activeTab !== 'campus_yearly' && (<>{activeTab !== 'campus_daily' && (<div className="flex bg-slate-100 rounded-lg p-1 border border-slate-200">{[{k:'annual',l:'年度'},{k:'monthly',l:'月度'},{k:'weekly',l:'週次'}].map(m=><button key={m.k} onClick={()=>setViewMode(m.k)} className={`px-3 py-1 text-xs font-medium rounded-md transition-all ${viewMode===m.k?'bg-white text-blue-600 shadow-sm':'text-slate-500 hover:text-slate-700'}`}>{m.l}</button>)}</div>)}
                                        {(viewMode !== 'annual' || activeTab === 'campus_daily') && (<div className="flex items-center bg-slate-100 rounded-lg px-3 py-1 border border-slate-200"><Calendar className="w-3 h-3 text-slate-500 mr-2" /><select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="bg-transparent border-none text-sm font-medium text-slate-700 focus:ring-0 cursor-pointer py-1 pr-1">{MONTHS_LIST.map(m => <option key={m} value={m}>{m}</option>)}</select></div>)}</>)}</>)}
                        <div className="flex items-center bg-slate-100 rounded-lg px-3 py-1 border border-slate-200"><MapPin className="w-3 h-3 text-slate-500 mr-2" /><select value={selectedCampusId} onChange={(e) => setSelectedCampusId(e.target.value)} className="bg-transparent border-none text-sm font-medium text-slate-700 focus:ring-0 cursor-pointer py-1"><option value="All">全校舎 (合計)</option><option disabled>──────────</option>{campusList.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                ...
                <div className="flex-1 overflow-y-auto p-6"><div className="max-w-7xl mx-auto space-y-6">
                        {activeTab === 'campus_daily' && (<div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 animate-in fade-in duration-500"><div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-bold text-slate-800 flex items-center"><Calendar className="w-5 h-5 mr-2 text-blue-600" />{selectedYear}年度 {selectedMonth} 日報カレンダー</h2><div className="text-xs text-slate-500 bg-slate-50 px-2 py-1 rounded">日付をクリックして入力・編集</div></div><div className="grid grid-cols-7 gap-px bg-slate-200 border border-slate-200 rounded-lg overflow-hidden">{['日','月','火','水','木','金','土'].map((d,i) => (<div key={d} className={`p-2 text-center text-xs font-bold ${i===0?'text-red-500':i===6?'text-blue-500':'text-slate-600'} bg-slate-100`}>{d}</div>))}{renderCalendar()}</div></div>)}
                        {activeTab === 'campus_yearly' && (<div className="space-y-6 animate-in fade-in duration-500"><div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100"><div className="flex justify-between items-center mb-6"><h2 className="text-lg font-bold text-slate-800 flex items-center"><FileText className="w-5 h-5 mr-2 text-blue-600" />年間計画入力 ({selectedCampusName} / {selectedYear}年度)</h2><button onClick={savePlanData} disabled={isSavingPlan} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center shadow-sm disabled:opacity-50"><Save className="w-4 h-4 mr-2" />{isSavingPlan ? '保存中...' : '計画を保存'}</button></div><div className="overflow-x-auto"><table className="w-full text-sm text-left border-collapse"><thead className="bg-slate-50 text-slate-600 font-medium border-b border-slate-200"><tr><th className="px-4 py-3 w-20">月度</th><th className="px-4 py-3 w-32">目標入会数</th><th className="px-4 py-3 w-32">体験会実施</th><th className="px-4 py-3 w-32">タッチ&トライ</th><th className="px-4 py-3 w-32">門配数</th><th className="px-4 py-3 w-24">入会率(%)</th></tr></thead><tbody className="divide-y divide-slate-100">{MONTHS_LIST.map((month) => (<tr key={month} className="hover:bg-slate-50"><td className="px-4 py-3 font-bold text-slate-700 bg-slate-50/50">{month}</td>{['enrollments', 'trials', 'touchTry', 'flyers'].map(field => (<td key={field} className="px-4 py-2"><input type="number" min="0" value={planData[month]?.[field] || 0} onChange={(e) => handlePlanChange(month, field, e.target.value)} className="w-full px-2 py-1 border border-slate-300 rounded focus:ring-2 focus:ring-blue-500 outline-none text-right" /></td>))}<td className="px-4 py-3 text-right font-medium text-slate-600">{planData[month]?.rate}%</td></tr>))}</tbody></table></div></div></div>)}
                        {activeTab === 'campus_weekly' && (<div className="space-y-6 animate-in fade-in duration-500"><div className="bg-white p-12 rounded-xl shadow-sm border border-slate-100 text-center"><div className="w-16 h-16 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-4"><Calendar className="w-8 h-8 text-blue-400" /></div><h3 className="text-xl font-bold text-slate-800 mb-2">月度計画（週次）入力</h3><p className="text-slate-500 max-w-md mx-auto">この機能は現在準備中です。将来的に、月ごとの目標を週単位にブレイクダウンして管理できるようになります。</p></div></div>)}
                        {activeTab === 'settings' && (<div className="space-y-6 animate-in fade-in duration-500"><div className="bg-white p-8 rounded-xl shadow-sm border border-slate-100"><h2 className="text-lg font-bold text-slate-800 mb-6 flex items-center"><School className="w-5 h-5 mr-2 text-blue-600" />校舎マスター管理</h2><div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8 items-end"><div className="md:col-span-1"><label className="block text-sm font-medium text-slate-700 mb-2">校舎ID <span className="text-xs text-red-500">*英数</span></label><input type="text" value={newCampusId} onChange={(e) => setNewCampusId(e.target.value)} placeholder="例: shibuya" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" /></div><div className="md:col-span-1"><label className="block text-sm font-medium text-slate-700 mb-2">校舎名</label><input type="text" value={newCampusName} onChange={(e) => setNewCampusName(e.target.value)} placeholder="例: 渋谷校" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" /></div><div className="md:col-span-2"><label className="block text-sm font-medium text-slate-700 mb-2">連携名 (スプレッドシート)</label><input type="text" value={newCampusSheetName} onChange={(e) => setNewCampusSheetName(e.target.value)} placeholder="例: 渋谷校" className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" /></div><div className="md:col-span-1"><button onClick={handleAddCampus} disabled={!newCampusId || !newCampusName} className="w-full px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center justify-center disabled:opacity-50"><Plus className="w-4 h-4 mr-2" />登録</button></div></div><div className="border-t border-slate-100 pt-6"><h3 className="text-sm font-bold text-slate-500 mb-4 uppercase tracking-wider">登録済み校舎一覧 ({campusList.length}校)</h3><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{campusList.map(c => (<div key={c.id} className="flex justify-between p-4 bg-slate-50 rounded-lg border border-slate-200"><div><div className="font-medium text-slate-700">{c.name}</div><div className="text-xs text-slate-400">ID: {c.id} | 連携: {c.sheetName}</div></div><button onClick={() => handleDeleteCampus(c.id, c.name)} className="text-slate-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button></div>))}</div></div></div></div>)}
                        {activeTab === 'summary' && (<div className="space-y-6 animate-in fade-in duration-500"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                    ...
                                    <StatCard title="体験会 (実施)" value={`${totals.trialExec || totals.trialLessons || 0}件`} subValue="実施ベース" trend={0} icon={Calendar} color="bg-amber-500" />
                                    <StatCard title="入会数" value={`${totals.newEnrollments}名`} subValue="新規獲得" trend={0} icon={Activity} color="bg-emerald-500" />
                                </div></div>)}
                        {activeTab === 'students' && (<div className="space-y-6 animate-in fade-in duration-500"><div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                                ...
                                </div><div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 h-[400px]"><h3 className="text-lg font-bold text-slate-800 mb-6">生徒数増減フロー</h3><ResponsiveContainer width="100%" height="100%"><BarChart data={displayData} stackOffset="sign"><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" /><YAxis padding={{ top: 20, bottom: 20 }} domain={['auto', 'auto']} /><Tooltip /><Legend /><ReferenceLine y={0} stroke="#000" /><Bar dataKey="newEnrollments" name="入会" fill="#10b981" stackId="stack" /><Bar dataKey="returns" name="復会" fill="#34d399" stackId="stack" /><Bar dataKey="transferIns" name="転入" fill="#06b6d4" stackId="stack" /><Bar dataKey="withdrawals_neg" name="退会" fill="#ef4444" stackId="stack" /><Bar dataKey="recesses_neg" name="休会" fill="#f59e0b" stackId="stack" /><Bar dataKey="transfers_neg" name="転校" fill="#f97316" stackId="stack" /><Bar dataKey="graduates_neg" name="卒業" fill="#a855f7" stackId="stack" /></BarChart></ResponsiveContainer></div>
                                <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden"><div className="p-6 border-b border-slate-100"><h3 className="text-lg font-bold text-slate-800">生徒数推移詳細</h3></div><div className="overflow-x-auto"><table className="w-full text-sm text-left"><thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-200"><tr><th className="px-4 py-3">期間</th><th className="px-4 py-3 text-emerald-600">入会</th><th className="px-4 py-3 text-emerald-600">復会</th><th className="px-4 py-3 text-cyan-600">転入</th><th className="px-4 py-3 text-rose-600">退会</th><th className="px-4 py-3 text-amber-600">休会</th><th className="px-4 py-3 text-orange-600">転校</th><th className="px-4 py-3 text-purple-600">卒業</th><th className="px-4 py-3 font-bold border-l border-slate-100">在籍生徒数</th></tr></thead><tbody className="divide-y divide-slate-100">{displayData.map((row, i) => (<tr key={i} className="hover:bg-slate-50"><td className="px-4 py-3 font-medium">{row.name}</td><td className="px-4 py-3">{row.newEnrollments}</td><td className="px-4 py-3">{row.returns}</td><td className="px-4 py-3">{row.transferIns}</td><td className="px-4 py-3">{row.withdrawals}</td><td className="px-4 py-3">{row.recesses}</td><td className="px-4 py-3">{row.transfers}</td><td className="px-4 py-3">{row.graduates}</td><td className="px-4 py-3 font-bold border-l border-slate-100">{row.totalStudents}</td></tr>))}</tbody></table></div></div></div>)}
                        {activeTab === 'marketing' && (<div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 h-[500px] animate-in fade-in duration-500"><div className="flex justify-between items-center mb-6"><h3 className="text-lg font-bold text-slate-800">集客ファネル</h3><span className="text-sm text-slate-500 bg-slate-100 px-3 py-1 rounded-full">{selectedCampusName} / {viewModeLabel}</span></div>
                                <ResponsiveContainer width="100%" height="100%"><ComposedChart data={displayData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" /><YAxis yAxisId="left" orientation="left" /><YAxis yAxisId="right" orientation="right" /><Tooltip /><Legend /><Bar yAxisId="left" dataKey="flyers" name="門配" fill="#94a3b8" /><Line yAxisId="right" type="monotone" dataKey="newEnrollments" name="入会数" stroke="#10b981" strokeWidth={3} /><Bar yAxisId="left" dataKey="trialApp" name="体験会予約" stackId="application" fill="#3b82f6" /><Bar yAxisId="left" dataKey="eventApp" name="イベント予約" stackId="application" fill="#93c5fd" /><Bar yAxisId="left" dataKey="trialExec" name="体験会実施" stackId="execution" fill="#f97316" /><Bar yAxisId="left" dataKey="eventExec" name="イベント実施" stackId="execution" fill="#fdba74" /></ComposedChart></ResponsiveContainer></div>)}
                    </div></div>
                {isInputModalOpen && (<div className="absolute inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm"><div className="bg-white rounded-xl shadow-2xl max-w-md w-full animate-in zoom-in-95 duration-200"><div className="flex justify-between items-center p-4 border-b border-slate-100"><h3 className="font-bold text-lg text-slate-800 flex items-center"><PenTool className="w-5 h-5 mr-2 text-blue-600" />日報入力 <span className="ml-2 text-sm font-normal text-slate-500">{reportDate}</span></h3><button onClick={() => setIsInputModalOpen(false)} className="text-slate-400 hover:text-slate-600"><X className="w-6 h-6" /></button></div><div className="p-6 space-y-6"><div><label className="block text-sm font-bold text-slate-700 mb-2">本日の天気</label><div className="grid grid-cols-5 gap-2">{[{id:'sunny',l:'晴',i:Sun,c:'text-orange-500'},{id:'cloudy',l:'曇',i:Cloud,c:'text-gray-500'},{id:'rainy',l:'雨',i:CloudRain,c:'text-blue-500'},{id:'snowy',l:'雪',i:Snowflake,c:'text-cyan-500'},{id:'closed',l:'休校',i:Ban,c:'text-rose-500'}].map(w=> (<button key={w.id} onClick={()=>handleWeatherSelect(w.id)} className={`flex flex-col items-center justify-center p-2 rounded-lg border transition-all ${dailyReportInput.weather===w.id?'bg-blue-50 border-blue-500 ring-1 ring-blue-500':'border-slate-200 hover:bg-slate-50'}`}><w.i className={`mb-1 w-5 h-5 ${w.c}`} /><span className="text-xs font-bold">{w.l}</span></button>))}</div></div>
                                <div className="grid grid-cols-2 gap-4"><div><label className="block text-xs font-bold text-slate-500 mb-1">門配 (枚)</label><input type="number" min="0" disabled={dailyReportInput.weather==='closed'} className="border rounded-lg w-full p-2 text-right font-mono text-lg focus:ring-2 focus:ring-blue-500 outline-none disabled:bg-slate-100 disabled:text-slate-400" value={dailyReportInput.flyers} onChange={e=>setDailyReportInput({...dailyReportInput,flyers:Number(e.target.value)})}/></div>
                                    ...
                                    <div className="col-span-2"><label className="block text-xs font-bold text-slate-500 mb-1">体験会実施 (回)</label><input type="number" min="0" disabled={dailyReportInput.weather==='closed'} className="border rounded-lg w-full p-2 text-right font-mono text-lg focus:ring-2 focus:ring-blue-500 outline-none disabled:bg-slate-100 disabled:text-slate-400" value={dailyReportInput.trialLessons} onChange={e=>setDailyReportInput({...dailyReportInput,trialLessons:Number(e.target.value)})}/></div></div><button onClick={handleSaveDailyReport} disabled={isSavingReport} className="w-full bg-blue-600 text-white py-3 rounded-lg font-bold shadow hover:bg-blue-700 transition-all flex items-center justify-center disabled:opacity-50">{isSavingReport ? <Loader2 className="animate-spin mr-2"/> : <Save className="mr-2"/>} 日報を保存</button></div></div></div>)}
            </main></div>);
}
