        ...
        {details && (
            <div className="mt-4 pt-3 border-t border-slate-100 space-y-1">
                {details.map((item, idx) => (
                    <div key={idx} className="flex justify-between text-xs text-slate-500"><span>{item.label}</span><span className="font-medium text-slate-700">{item.value}名</span></div>
                ))}
            </div>
        )}
    </div>
...
function RobotSchoolDashboard() {
    ...
    const [isCampusMenuOpen, setIsCampusMenuOpen] = useState(true); // ★修正：初期値をセット
    ...
    const fetchFromFirebaseAndCache = async () => {
        ...
        try {
            ...
            setCampusList(data.campuses); setRealEnrollments(data.enroll); setRealStatusChanges(data.status); setRealTransfers(data.transfers); setRealDailyReports(data.reports); setRealTrialApps(data.trials);
            const now = new Date(); setLastUpdated(now);
    ...
    const generateAllCampusesData = (targetCampuses, enrollList, statusList, transferList, reportList, trialAppsList, targetYear) => {
        ...
        const findCampusId = (dbName) => {
            ...
            for (const [id, names] of Object.entries(campusMatchMap)) {
                if (names.some(name => normDbName.includes(name) || name.includes(normDbName))) return id;
            }
            return null;
        };
        ...
        targetCampuses.forEach(campusObj => {
            ...
            dataMap[campusId] = MONTHS_LIST.map((month, mIdx) => {
                ...
                const daily = Array.from({ length: daysInMonth }, (_, dIdx) => {
                    ...
                    const countDay = (list, type = null) => list.filter(i => {
                        const d = parseDate(i.date);
                        return findCampusId(i.campus) === campusId && d && formatDateStr(d) === dateStr && (!type || (i.type && i.type.includes(type)));
                    }).length;

                    const dEnr = countDay(enrollList); const dTrIn = countDay(transferList);
                    const dWith = countDay(statusList, "退会"); const dTrOut = countDay(statusList, "転校");
                    const dGrad = countDay(statusList, "卒業"); const dRec = countDay(statusList, "休会");
                ...
                return { name: month, ...Object.fromEntries(Object.keys(daily[0]).filter(k => k!=='name').map(k => [k, sum(k)])), totalStudents: currentStudents, daily, weekly };
            });
        });
        ...
        <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900">
            <aside className="w-64 bg-slate-900 text-white flex flex-col shrink-0 overflow-y-auto">
                ...
                <nav className="flex-1 py-4 px-3 space-y-1">
                    <button onClick={() => {setActiveTab('summary'); setSelectedCampusId('All');}} className={`w-full flex items-center space-x-3 px-3 py-3 rounded-lg ${activeTab === 'summary' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}><LayoutDashboard className="w-5 h-5" /><span>経営サマリー</span></button>
                    <button onClick={() => setActiveTab('students')} className={`w-full flex items-center space-x-3 px-3 py-3 rounded-lg ${activeTab === 'students' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}><Users className="w-5 h-5" /><span>生徒管理</span></button>
                    <button onClick={() => setActiveTab('marketing')} className={`w-full flex items-center space-x-3 px-3 py-3 rounded-lg ${activeTab === 'marketing' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}><Megaphone className="w-5 h-5" /><span>集客・販促</span></button>
                    <div className="pt-4 pb-2 px-3 text-xs font-semibold text-slate-500 uppercase flex justify-between items-center cursor-pointer" onClick={() => setIsCampusMenuOpen(!isCampusMenuOpen)}><span>校舎管理</span><ChevronDown className={`w-4 h-4 transition-transform ${isCampusMenuOpen ? 'rotate-180' : ''}`} /></div>
                    {isCampusMenuOpen && (
                ...
                <div className="p-4 border-t border-slate-800 text-[10px] text-slate-500">最終更新: {lastUpdated?.toLocaleTimeString()}</div>
            </aside>
                    ...
                    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                        <div className="bg-white rounded-xl shadow-2xl max-w-md w-full animate-in zoom-in-95">
                            ...
                            <div className="p-6 space-y-6">
                                ...
                                <button onClick={handleSaveDailyReport} disabled={isSavingReport} className="w-full bg-blue-600 text-white py-3 rounded-lg font-bold shadow hover:bg-blue-700 transition-all flex items-center justify-center disabled:opacity-50">{isSavingReport ? <Loader2 className="animate-spin mr-2"/> : <Save className="mr-2"/>} 保存</button>
                            </div>
                        </div>
                    </div>
                )}
